...
import { ArrowDownRight, ArrowUpRight, Atom, Award, Beaker, ChevronDown, Github, Globe2, Linkedin, Mail, MapPin, Menu, MoveUpRight, Orbit, Send, Sun, Moon, X } from 'lucide-react';
...
export default function ResearchSite() {
  ...
  return (
    <main className={isDark ? 'dark-theme' : 'light-theme'}>
      ...
      <nav>
        <a href="#top" className="brand">MY<span>·</span></a>
        <div className="nav-links">{['About','Research','Work','Contact'].map(x => <a key={x} href={`#${x.toLowerCase()}`}>{x}</a>)}</div>
        ...
        <a className="nav-contact" href="mailto:yousufmuhammedkpm@gmail.com">Let’s connect <MoveUpRight size={14}/></a>
...
>
  ...
  <MagneticButton href="#contact" dark>
    Start a conversation
      ...
      <section id="contact" className="contact section"><Reveal><div className="eyebrow"><span />08 — Contact</div><h2>Let’s make the air<br/><em>more legible.</em></h2><p>For research collaborations, speaking invitations, and thoughtful conversations.</p><div className="contact-links"><a href="mailto:yousufmuhammedkpm@gmail.com"><Mail/>yousufmuhammedkpm@gmail.com</a><a href="#top"><Linkedin/>LinkedIn</a><a href="#top"><Github/>GitHub</a></div></Reveal><Reveal><form onSubmit={submit}><label>Your name<input required placeholder="How should I address you?" /></label><label>Email address<input required type="email" placeholder="you@institution.edu" /></label><label>Message<textarea required placeholder="Tell me a little about what’s on your mind..." /></label><button className="submit" type="submit">{sent ? 'Message prepared — thank you.' : <>Send a message <Send size={16}/></>}</button></form></Reveal></section>
      <footer><a href="#top" className="brand">MY<span>·</span></a><p>© {new Date().getFullYear()} Muhammed Yousuf S Designed for clearer skies.</p><a href="#top">Back to top <ArrowUpRight size={15}/></a></footer>
    </main>
  );
}
