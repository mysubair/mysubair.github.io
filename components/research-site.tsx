...
export default function ResearchSite() {
  ...
  {theme === 'dark' ? <Sun size={13} /> : <Moon size={13} />}
</span></button><a className="nav-contact" href="mailto:yousuf@iitd.ac.in">Let’s connect <MoveUpRight size={14}/></a></div><button className="menu" onClick={() => setMenu(!menu)} aria-label="Open menu">{menu ? <X/> : <Menu/>}</button></nav>
    ...
    <section id="contact" className="contact section"><Reveal><div className="eyebrow"><span />08 — Contact</div><h2>Let’s make the air<br/><em>more legible.</em></h2><p>For research collaborations, speaking invitations, and thoughtful conversations.</p><div className="contact-links"><a href="mailto:yousuf@iitd.ac.in"><Mail/>yousuf@iitd.ac.in</a><a href="#top"><Linkedin/>LinkedIn</a><a href="#top"><Github/>GitHub</a></div></Reveal><Reveal><form onSubmit={submit}><label>Your name<input required placeholder="How should I address you?" /></label><label>Email address<input required type="email" placeholder="you@institution.edu" /></label><label>Message<textarea required placeholder="Tell me a little about what’s on your mind..." /></label><button className="submit" type="submit">{sent ? 'Message prepared — thank you.' : <>Send a message <Send size={16}/></>}</button></form></Reveal></section>
    <footer><a href="#top" className="brand">MY<span>·</span></a><p>© {new Date().getFullYear()} Muhammed Yousuf. Designed for clearer skies.</p><a href="#top">Back to top <ArrowUpRight size={15}/></a></footer>
  </main>;
}
